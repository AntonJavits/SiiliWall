...
export const addShortForm = (addedShortForm, projectId) => {
    const projectIdForCache = `Project:${projectId}`;
    const { shortform } = client.readFragment({
      id: projectIdForCache,
      fragment: PROJECTS_SHORTFORMS,
    });
    const newShortForms = shortform.concat(addedShortForm);
    client.writeFragment({
      id: projectIdForCache,
      fragment: PROJECTS_SHORTFORMS,
      data: {
        shortforms: newShortForms,
      },
    });
  };
...
export const restoreProjectToCache = (projectId) => {
    const projectToBeRestored = `Project:${projectId}`
    const projectIdForCache = `Project:${projectId}`
    ...
    client.cache.evict({ id: projectToBeRestored })
}
